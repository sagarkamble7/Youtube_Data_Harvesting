...
def channel_table():
    ...
    for index, row in df.iterrows():
        formatted_publish_date = row['piblish_date'].replace(
            'T', ' ').replace('Z', '')
        insert_query = '''
...
def playlist_table():
   ...
   # Fetch playlist data from MongoDB
    ...
    for index, row in df1.iterrows():
        formatted_published_at = row['publishedAt'].replace(
            'T', ' ').replace('Z', '')
...
def comments_table():
    ...
    for index, row in df3.iterrows():
        formatted_published_at = row['comment_published'].replace(
            'T', ' ').replace('Z', '')
...
if st.button('Migrate to SQL'):
    Table = tables()
    st.success('Table Created in Database Successfully')

show_table = st.radio('SELECT THE TABLE FOR THE VIEW',
...
if question == '1. All the videos and the channel':
    query1 = '''select video_title as videoName, channel_name as ChannelName from videos order by channel_name'''
    ...
    query3 = '''select video_title as Video_title ,channel_name as Channel_name, views as Views from videos order by views desc limit 10'''
    ...
    query4 = '''select comments as Comment_Count, video_title as Video_Name from videos order by comments desc'''
    ...
    mydb.commit()
    df5 = pd.DataFrame(
        t5, columns=["Video Name", "Number Of Likes", "Channel Name"])
    st.write(df5)

elif question == "6. Likes Of Each Video And Thier corresponing Video Names":
    query6 = '''select likes as Total_Likes, video_title as Video_name from Videos order by likes desc'''
    ...
    query7 = "select channel_name as Channel_name, view_count as Total_Views from channels order by view_count desc"
    ...
    query8 = "select channel_name as Channel_name, count(video_title) as number_of_videos from videos where extract(year from Publishedat) = 2022 group by channel_name order by number_of_videos desc"
    cursor.execute(query8)
    t8 = cursor.fetchall()
    mydb.commit()
    df8 = pd.DataFrame(t8, columns=["channel_name", "Video Count"])
    st.write(df8)

elif question == '9. average duration of video and their channel name':
    query9 = '''select floor(avg(regexp_replace(replace(replace(replace(video_duration, "PT",""), "M", ":"), "S", ""), '[A-Z, a-z]', ""))) as average_duration, channel_name as channel_name from videos group by channel_name order by average_duration desc'''
    ...
    mydb.commit()
    df10 = pd.DataFrame(
        t10, columns=["video name", "comment count", "channel name"])
    st.write(df10)
